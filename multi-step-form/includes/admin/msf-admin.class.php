		...
		if (isset($_FILES['json-import'])) {
			...
			if (isset($uploaded['error'])) {
				...
				if (json_last_error() !== JSON_ERROR_NONE) {
					$this->notice('error', __('Invalid JSON file. Please check your file format.', 'multi-step-form'));
					wp_delete_file($uploaded['file']); // Clean up uploaded file
					return;
				}
				
				// SECURITY FIX: Check for required JSON structure
				if (!isset($json_data['wizard']) || !is_array($json_data['wizard'])) {
					$this->notice('error', __('Invalid form structure. The JSON file must contain a valid form configuration.', 'multi-step-form'));
					wp_delete_file($uploaded['file']); // Clean up uploaded file
					return;
				}
				
				// All validations passed, proceed with import
				$this->import_json($content);
			}
		}
	}
